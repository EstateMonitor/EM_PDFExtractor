...
def select_extract_data(rect, r, g, b, page):
...
def highlight_sorted_drawings(pdf_path, output_path):
    ...
    for index, height in enumerate(sorted_heights):
        ...
        for page_num, rect in drawings_dict[height]:
            ...
            # company_name of OOO
            company_name = select_extract_data(fitz.Rect(rect.x0 + 90, rect.y0 - 15, rect.x1 - 130, rect.y1), 0,1,0, page)
            ...
            start_date = select_extract_data(fitz.Rect(rect.x0 - 65, rect.y0 + 5, rect.x1 - 740, rect.y1 + 35), 1,0,0, page)
            print(f"Extracted start_date in rectangle: {start_date}")

            start_time = select_extract_data(fitz.Rect(rect.x0 - 8, rect.y0 + 5, rect.x1 - 695, rect.y1 + 35), 0, 0, 1, page)
            print(f"Extracted start_time in rectangle: {start_time}")

            end_date = select_extract_data(fitz.Rect(rect.x0 + 37, rect.y0 + 5, rect.x1 - 650, rect.y1 + 35), 1, 0, 0, page)
            print(f"Extracted end_date in rectangle: {end_date}")

            end_time = select_extract_data(fitz.Rect(rect.x0 + 82, rect.y0 + 5, rect.x1 - 605, rect.y1 + 35), 0, 0, 1, page)
            print(f"Extracted end_time in rectangle: {end_time}")

            downtime_hours = select_extract_data(fitz.Rect(rect.x0 + 127, rect.y0 + 5, rect.x1 - 565, rect.y1 + 35), 1, 0, 0, page)
            print(f"Extracted end_date in rectangle: {downtime_hours}")

            factory_number = select_extract_data(fitz.Rect(rect.x0 + 168, rect.y0 + 5, rect.x1 - 515, rect.y1 + 35), 1, 0, 1, page)
            print(f"Extracted end_date in rectangle: {factory_number}")

            registration_number = select_extract_data(fitz.Rect(rect.x0 + 222, rect.y0 + 5, rect.x1 - 455, rect.y1 + 35), 1, 1, 0, page)
